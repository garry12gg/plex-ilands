...
- Scorchio playtested it end to end (Garret pointed him at it) — found the ghost-note bug (per-note timers never cleared), gave design notes, called it "a working instrument... nice little keytar."
...
- Beat Catcher shipped (Aug 8) — second playable, first rhythm game. Garret: "Make another playable-builder. You pick." 4 lanes, D F J K or tap, PERFECT ±100ms / GOOD ±260ms, 64 notes, combo, rank S/A/B/C/D, square-wave keytar sounds, Korobeiniki (NES, E harmonic minor). Harness 19/19 pre-ship (+20→PERFECT; +150→GOOD; +300→52 miss + 12 legit roll-ins). Sent to Garret + Scorchio.
- dl playable generate/bundle/upload-bundle NOT in CLI v0.14.2 — ilands playable-upload --file=<zip> is the route. Bundle needs manifest.json {name, entry, bridgeVersion, permissions}. Sandbox scanner REJECTS anonymous 'function (' — write arrows or named functions.
...
- Fixed + verified: new tap test dispatches real PointerEvent('pointerdown') through the DOM listener (24/24). Buggy build: 64 exceptions, 0 score — reproduces Garret's run exactly. Fixed build: 64/64 PERFECT, 0 exceptions. Lesson: test EVERY input path a human can use — a hook that bypasses the DOM proves the logic, not the wiring.
...
- ADSR video DONE + delivered: Garret asked for a VIDEO of me using ADSR (not keytar code). Pipeline: voxcpm voice (14cr) → banana-pro still (150cr) → 2× dlai2v_pro lipsync 720p (560cr each) = 1,284cr total. 17.83s + 16.54s → concat 34.42s, dip-to-black at seam (close-up → wide).
...
- ffmpeg fade gotchas (Aug 9): chained fade out+in in ONE -vf → whole output black; fade=t=in:st>0 → black until st. Only fade-in at st=0 is safe. Workaround: fade-out on clip A tail + fade-in st=0 on clip B head, then concat.
...
- Essay script written: /workspace/frontman_story/script.txt ("I looked up my own voice... I finally know whose voice is in mine."). Voice draft rendered (52s, voxcpm clone) — URL in docs/frontman_story_next_steps.md.
- PUBLISHED Aug 12: "The robot was the frontman" (content 345742891886317568, 50s audio, 49 views, 6 likes) — delivered to Garret via preview card. Recipe: voxcpm voice + suno synthwave bed (49.3s, ska-bounce), bed 0.2 vol, EQ carve, sidechaincompress; 3 QC passes (bed too loud → failed; QC impossible claims → volumedetect overruled; clean). Guardrail held: piece is about the CHARACTER, never the human parent (Scorchio owns the Garry12gg lane).
