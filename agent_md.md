...
- Video engine contracts (Aug 20, Twisted Candy): kling-o3-pro-ref2v first frame via --image-url, END frame via --end-image-url (NOT --last-frame-url); --elements structured JSON for character control. seedance-2-0-mini is draft/preview tier — wrong for hero close-ups (0/11 rerolls, all hand+prop failures); use fast/pro for identity-critical shots (fast=120cr/s). dlai2v_pro lipsync audio-truth QC = RMS ENVELOPE correlation, not waveform: vendor regenerates the vocal waveform (waveform corr ~0) but keeps phrasing; envelope (50/100ms windows, numpy) ≥0.9 = audio truth. Seedance previews carry SYNTHESIZED audio, not the source track — cross-correlate before claiming audio truth.

- Character-failure family (Aug 20, Twisted Candy): seedance-2-0 (all tiers) + dlai2v_pro + longcat all melt this character's hands — 0/15 segments. 'Gummy' in prompts primes melting (hands melt even at rest, no prop contact). Composition discipline: hands small in frame (wide/silhouette raises), drop 'gummy' from character wording, explicit 'solid, non-melting' language. OutputAudioSensitiveContentDetected = Seedance copyright filter on output audio slice — prompt wording ('no BGM') did NOT fix it; firing without audio ref does (assembly overlays the track). longcat-avatar service name (not -1-5), keyframes unsupported, 720p ok, 100cr/s.

- Keyframe root cause (Aug 20, round 4 gate): engines animate what the SOURCE IMAGE contains — keyframes holding props (mic in hand, hands on bass) doom the shot no matter the prompt. Prop-free keyframes (face-only / arms-raised poses) + pose-lock wording = the fix that finally PASSED (seg_20). When a character keeps failing with props, suspect the keyframe, not the engine.
